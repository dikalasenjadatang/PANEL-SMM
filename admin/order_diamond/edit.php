...
if (isset($_SESSION['user'])) {
	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		if (isset($_GET['oid'])) {
			$post_oid = $_GET['oid'];
			$checkdb_order = mysqli_query($db, "SELECT * FROM orders_diamond WHERE oid = '$post_oid'");
			$datadb_order = mysqli_fetch_assoc($checkdb_order);
			if (mysqli_num_rows($checkdb_order) == 0) {
				header("Location: ".$cfg_baseurl."admin/orders_diamond.php");
				...
				if (isset($_POST['edit'])) {
					...
					if ($post_status != "Pending" AND $post_status != "Processing" AND $post_status != "Error" AND $post_status != "Partial" AND $post_status != "Success") {
						...
						$update_order = mysqli_query($db, "UPDATE orders_diamond SET status = '$post_status' WHERE oid = '$post_oid'");
								...
								<div class="card">
									...
									<div class="card-body">
										...
										<form class="form-horizontal" role="form" method="POST">
											...
											<a href="<?php echo $cfg_baseurl; ?>admin/orders_diamond.php" class="btn btn-info btn-bordered waves-effect w-md waves-light">Kembali ke daftar</a>
